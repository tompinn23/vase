...
from collections.abc import AsyncGenerator, MutableMapping, AsyncIterable
...
class Journal:
    ...
    async def parse_entry(
        self, line: bytes
    ) -> MutableMapping[str, Any] | None | dict[str, None]:  # noqa: C901, CCR001
        """
        ...
        try:
            ...
            if event_type == "fileheader":
                ...
                if entry["StarSystem"] == "ProvingGround":
                    ...
                    logger.debug(
                        f"Updating system name {self.cmdr} {entry['StarSystem']}"
                    )
    ...
    async def _read_navroute(
        ...
        try:
            async with await anyio.open_file(
                self.journal_dir / "NavRoute.json", "rb"
            ) as f:
                raw = await f.read()
        except FileNotFoundError:
    ...
    async def _read_outfitting(
        self, ignore_timestamp: bool = False
    ) -> dict[str, Any] | None:
        """Read and parse Outfitting.json."""
        try:
            async with await anyio.open_file(
                self.journal_dir / "Outfitting.json", "rb"
            ) as f:
                raw = await f.read()
        except FileNotFoundError:
            return None
    ...
    async def _read_shipyard(
        ...
        try:
            async with await anyio.open_file(
                self.journal_dir / "Shipyard.json", "rb"
            ) as f:
                raw = await f.read()
        except FileNotFoundError:
            return None
    ...
    async def _read_market(
        ...
        try:
            async with await anyio.open_file(
                self.journal_dir / "Market.json", "rb"
            ) as f:
                raw = await f.read()
        except FileNotFoundError:
            # logger.warning("Couldn't open NavRoute.json.")
    ...
    async def _read_fcmaterials(
        ...
        try:
            async with await anyio.open_file(
                self.journal_dir / "FCMaterials.json", "rb"
            ) as f:
                raw = await f.read()
        except FileNotFoundError:
            # logger.warning("Couldn't open NavRoute.json.")
